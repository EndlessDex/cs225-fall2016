...
gradientColorPicker::gradientColorPicker(RGBAPixel fadeColor1,
    ...
    :color1(fadeColor1), color2(fadeColor2), rad(radius), midX(centerX), midY(centerY) {}
...
{
    int d = abs(midX - x) + abs(midY - y);
    RGBAPixel retVal = color2;
    if(d < rad) {
        retVal.red = color1.red - floor(d*color1.red / rad) + floor(d*color2.red / rad);
        retVal.green = color1.green - floor(d*color1.green / rad) + floor(d*color2.green / rad);
        retVal.blue = color1.blue - floor(d*color1.blue / rad) + floor(d*color2.blue / rad);
    }
    return retVal;
}
