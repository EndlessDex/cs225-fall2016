...
size_t heap<T, Compare>::maxPriorityChild(size_t currentIdx) const
{
    ...
    if(leftChild(currentIdx) < _elems.size()) {
        if(rightChild(currentIdx) < _elems.size()) {
            bool higher = higherPriority(_elems[leftChild(currentIdx)],
                                      _elems[rightChild(currentIdx)]);
            return higher ? leftChild(currentIdx) : rightChild(currentIdx);
        }
        //If no right child, return leftIdx
        return leftChild(currentIdx);
    }
    //If no left child, assume right child exists and return rightIdx
